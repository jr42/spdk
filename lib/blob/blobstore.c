...
static void
blob_bitmap_cpl(struct spdk_blob_bitmap_ctx *ctx, int rc)
{
...
static void
blob_bitmap_unfreeze_cpl(void *cb_arg, int rc)
{
	...
	blob_bitmap_cpl(ctx, rc);
}

static void
blob_create_bitmap(struct spdk_blob_bitmap_ctx *ctx)
{
	struct spdk_blob *blob = ctx->blob;
	uint64_t i;
...
static void
blob_bitmap_freeze_cpl(void *cb_arg, int rc)
{
	struct spdk_blob_bitmap_ctx *ctx = (struct spdk_blob_bitmap_ctx *)cb_arg;
	...
	blob_create_bitmap(ctx);
	blob_unfreeze_io(ctx->blob, blob_bitmap_unfreeze_cpl, ctx);
}
			     ...
			     void *cb_arg)
{
	...
	if (blob->data_ro && blob->md_ro) {
		blob_create_bitmap(ctx);
		blob_bitmap_cpl(ctx, 0);
		return 0;
	}

	blob->locked_operation_in_progress = true;
	blob_freeze_io(blob, blob_bitmap_freeze_cpl, ctx);
