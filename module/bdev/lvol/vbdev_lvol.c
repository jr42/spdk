			...
			spdk_lvs_reset_completion_cb cb_fn, void *cb_arg)
{
	...
	SPDK_DEBUGLOG(bdev, "starting reset new change");
	args = calloc(1, sizeof(*args));
	if (!args) {
		return -ENOMEM;
	}
	...
	return spdk_bs_bdev_reset(bs_dev, lvs_bs_dev_reset_cb, args);
}
				...
				void *cb_arg)
{
	...
	rc = spdk_lvol_create_with_opts(lvs, lvol_opts, _vbdev_lvol_create_cb, req);
	if (rc != 0) {
		free(req);
	}
